...
export default function WaveFunctionCollapseBgV6({ className }: WaveFunctionCollapseBgV6Props) {
  ...
  useEffect(() => {
      ...
      for (let y = cy - radius; y <= cy + radius; y++) {
        for (let x = cx - radius; x <= cx + radius; x++) {
          if (x >= 0 && x < wfcCols && y >= 0 && y < wfcRows) {
            ...
            wfcCells[i].options = Array.from({ length: wfcTiles.length }, (_, k) => k);
            wfcCells[i].tileId = -1;
            wfcCells[i].fadeIn = 0;
            uncollapsedIndices.add(i);
          }
    ...
    function collapseWfc() {
      ...
      if (boids.length < MAX_BOIDS && Math.random() < 0.2) {
        ...
        const cy = Math.floor(chosen / wfcCols) * WFC_CELL_SIZE + WFC_CELL_SIZE / 2;
        spawnBoid(cx, cy);
      }
    }
    ...
    // --- Game of Life Logic ---
    function resetGol() {
      golGrid = new Array(golCols).fill(null).map(() => new Array(golRows).fill(0));
    ...
    function tickGol() {
      const next = new Array(golCols).fill(null).map(() => new Array(golRows).fill(0));
      let aliveCount = 0;
      ...
      if (aliveCount < 5) {
        for (let i = 0; i < 10; i++) {
           golGrid[Math.floor(Math.random() * golCols)][Math.floor(Math.random() * golRows)] = 1;
        }
      }
    }
    ...
    function updateBoids() {
      for (let i = 0; i < boids.length; i++) {
        ...
        if (count > 0) {
            ...
            if (aliLen > 0) { aliX = (aliX / aliLen) * BASE_SPEED; aliY = (aliY / aliLen) * BASE_SPEED; }
            
            cohX = (cohX / count) - b.x; cohY = (cohY / count) - b.y;
            const cohLen = Math.sqrt(cohX * cohX + cohY * cohY);
            if (cohLen > 0) { cohX = (cohX / cohLen) * BASE_SPEED; cohY = (cohY / cohLen) * BASE_SPEED; }
        ...
        let foundGol = false;
        let golAttractX = 0, golAttractY = 0;
        ...
        for (let di = -2; di <= 2; di++) {
            for (let dj = -2; dj <= 2; dj++) {
               const checkX = gX + di;
               const checkY = gY + dj;
               if (checkX >= 0 && checkX < golCols && checkY >= 0 && checkY < golRows) {
        ...
        } else if (speed < 1.0) { // Keep them moving
            b.vx = (b.vx / speed) * 1.0;
            b.vy = (b.vy / speed) * 1.0;
        }
                ...
                for (let i = 0; i < wfcCells.length && faded < 3; i++) {
                    ...
                    if (wfcCells[rIdx].tileId !== -1) {
                        ...
                        if (wfcCells[rIdx].fadeIn <= 0) {
                            ...
                            wfcCells[rIdx].options = Array.from({ length: wfcTiles.length }, (_, k) => k);
